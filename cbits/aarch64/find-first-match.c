...
// Based on 
// http://0x80.pl/articles/simd-strfind.html#arm-neon-32-bit-code and 
// https://github.com/WojciechMula/sse4-strstr/blob/master/aarch64-strstr-v2.cpp
static inline ptrdiff_t mula (uint8_t const * const needle,
                              size_t const needle_len,
                              uint8_t const * haystack,
                              size_t const haystack_len) {
  uint8x16_t const mask_first = vdupq_n_u8(needle[0]);
  uint8x16_t const mask_last = vdupq_n_u8(needle[needle_len - 1]);
  ...
  for (size_t i = 0; i < big_strides; i++) {
    uint8x16_t const input_start = vld1q_u8(ptr);
    uint8x16_t const input_end = vld1q_u8(ptr + needle_len - 1);
    uint8x16_t const results = vandq_u8(vceqq_u8(input_start, mask_first),
                                        vceqq_u8(input_end, mask_last));
    // Unpack the first block and check by hand.
    uint64_t mask = vgetq_lane_u64(vreinterpretq_u64_u8(results), 0);
    if (mask != 0) {
      for (size_t j = 0; j < 8; j++) {
        // Check if the byte is set.
        if ((mask & 0xFF) != 0) {
          // Because we know the ends match, only need to check the middle.
          if (memcmp(ptr + j + 1, needle + 1, needle_len - 2) == 0) {
            return (i * 16) + j;
          }
        }
        // 'Shift off' a byte.
    ...
    // If we didn't find anything, try the second block.
    //
    // This works the same way as above, but with an additional offset of 8.
    mask = vgetq_lane_u64(vreinterpretq_u64_u8(results), 1);
    if (mask != 0) {
      for (size_t j = 0; j < 8; j++) {
        if ((mask & 0xFF) != 0) {
          if (memcmp(ptr + j + 9, needle + 1, needle_len -2) == 0) {
            return (i * 16) + j + 8;
          }
        }
        mask >>= 8;
